...
class ActiveStatusList(generics.ListAPIView):
    serializer_class = ActiveStatusViewSerializer
    # Requires a valid JWT token for access
    permission_classes = [permissions.IsAuthenticated]
    pagination_class = CustomPagination

    def get_queryset(self):
        queryset = ActiveStatus.objects.filter(status=True).order_by('id')
        # try:
        #     institution_id = self.request.user.institution
        #     branch_id = self.request.user.branch
        #     # users = Authentication.objects.get(id=user_id)
...
class LeaveApprovalStatusList(generics.ListAPIView):
    serializer_class = LeaveApprovalViewSerializer
    # Requires a valid JWT token for access
    permission_classes = [permissions.IsAuthenticated]
    pagination_class = CustomPagination
