...
class staffLeaveTransactionDelete(generics.UpdateAPIView):
    queryset = StaffLeaveTransaction.objects.all()
    serializer_class = StaffLeaveTransactionCreateSerializer
    permission_classes = [permissions.IsAuthenticated]  # Requires a valid JWT token for access
    ...
    def partial_update(self, request, *args, **kwargs):
        '''Check user has permission to Delete start'''
        permission_check = check_permission(self.request.user.id, 'Apply Leave', 'delete')
        if not permission_check:
            return CustomResponse(code=status.HTTP_401_UNAUTHORIZED, message="Permission denied", data=None)
        '''Check user has permission to Delete End'''
        ...
        instance = self.get_object()        
        if not instance.status:
            return CustomResponse(code=status.HTTP_400_BAD_REQUEST, message=f"Leave {instance.code} already Deleted", data=None)
        # Update the "status" field to False
        if instance.app_status.type=='SUBMITTED' and (request.user==instance.apply_by.user or request.user==instance.responsible.user):
            instance.status = False
            instance.save()
            # Customize the response format for successful update
            return CustomResponse(code=status.HTTP_200_OK, message=f"Leave {instance.code} Delete successfully", data=None)
        else:
            return CustomResponse(code=status.HTTP_401_UNAUTHORIZED, message=f"You are not able to delete this leave", data=None)
...
class StaffStaffCreate(generics.ListCreateAPIView):
...
class StaffUpDownCreateView(generics.ListCreateAPIView):
    # queryset = Version.objects.filter(status=True).order_by('id')
    serializer_class = StaffUpDownViewSerializer
    permission_classes = [permissions.IsAuthenticated]  # Requires a valid JWT token for access
    pagination_class = CustomPagination
    
    def get_queryset(self):
        queryset = StaffPayroll.objects.filter(status=True).order_by('-id')
        try:
            institution_id = self.request.user.institution
            branch_id = self.request.user.branch
            # users = Authentication.objects.get(id=user_id)
    ...
    def create(self, request, *args, **kwargs):
        ...
        #     return CustomResponse(code=status.HTTP_401_UNAUTHORIZED, message="Permission denied", data=None)
        '''Check user has permission to View end'''
        serializer_class = StaffUpDownCreateSerializer
        serializer = serializer_class(data=request.data)
        try:
            if serializer.is_valid():
                institution_data = serializer.validated_data.get('Institution')
                ...
                print(staff.institution)
                StaffPayroll.objects.filter(staff=staff,is_active=True,status=True).update(is_active=False)
                # If data is provided, use it; otherwise, use the values from the request user
                # institution_info = institution_data if institution_data is not None else self.request.user.institution
                # branch_info = branch_data if branch_data is not None else self.request.user.branch
                
                instance = serializer.save(institution=staff.institution, branch=staff.branch)
                # Customize the response data
                return CustomResponse(code=status.HTTP_200_OK, message="Staff Up or Down created successfully", data=StaffUpDownViewSerializer(instance).data)
            # If the serializer is not valid, return an error response
            return CustomResponse(code=status.HTTP_400_BAD_REQUEST, message="Validation error", data=serializer.errors)
        except Exception as e:
            # Handle other exceptions
